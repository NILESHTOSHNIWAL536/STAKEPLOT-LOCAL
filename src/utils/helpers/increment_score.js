...
const postScoreCounter = async (userId, postId, isDelete) => {
  try {
    ...
    if (isDelete) {
      const matchedPost = user.pendingPosts.find((post) => post.postId.toString() === postId.toString());
...
const handleDailyCounter = async (userId, counterField, scoreToAdd, objectId, incScoreCount, countBreak) => {
  try {
    ...
    let dailyCounter = user[counterField].find((counter) => counter.date === currentDate);
    if (!dailyCounter) {
    ...
    let dailyClaimCount = user[dailyClaimCountString].find((counter) => counter.date === currentDate);
    ...
    const find = objectId == '' || transactionList?.some((id) => id?.toString() === objectId?.toString());
    ...
    user[counterField].find((tag) => tag.date === currentDate).count = dailyCounter.count;
    await user.save();
...
const handleDailyClaimCount = async (userId, counterField, countBreak) => {
  try {
    const currentDate = getCurrentDate();
    let user = await UserActivity.findOne({ userId });
