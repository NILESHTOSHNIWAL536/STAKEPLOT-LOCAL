...
function getInsights(data) {
  ...
  if (weeklySnack > 0.08 * monthlyTotal) {
    insights.push(
      `Arre, you spent ${formatINR(weeklySnack)} on snacks this week – that's a lot of munching! Be careful, your budget might cry! 😮`
    );
  }
  if (weeklyPersonalCare > 0.2 * monthlyTotal) {
    insights.push(
      `Your ${formatINR(weeklyPersonalCare)} Personalcare spree this week? That's basically a VIP ticket to Splurge City! 🛍️`
    );
  }
  if (weeklyCommerce > 0.01 * monthlyTotal) {
    insights.push(
      `You spent over ${formatINR(weeklyCommerce)} on commerce this week – that's a mini-vacation fund 😅`
    );
  }
  ...
  if (previousTravel && currentTravel < previousTravel * 0.6) {
    insights.push(
      "Travel expenses dropped by 40% this month – working from home, eh?"
    );
  }
  ...
  if (entertainment > totalIncome * 0.15) {
    insights.push(
      `${formatINR(entertainment)} on entertainment – treat yourself, but maybe slow down next week?`
    );
  }
  ...
  if (currentWeekTotal < lastWeekTotal) {
    insights.push(
      ...
      )} compared to last week – that’s impressive! One step at a time..`
    );
  }
  ...
  if (previousChaiBudget > 400) {
    insights.push(
      `Monthly chai budget = ${formatINR(previousChaiBudget)} ☕ – desi priorities on point.`
    );
  }
  ...
  if (zomato === 0 && swiggy === 0) {
    insights.push(
      `₹0 spent on Zomato and Swiggy this week – chef ${username} in the house? 👨‍🍳`
    );
  } else {
    insights.push(
      `You spent ${formatINR(
        zomato + swiggy
      )} on Zomato and Swiggy this week – chef ${username} in the house? 👨‍🍳`
    );
  }
  ...
  if (Array.isArray(daily)) {
    ...
    if (noSpendDays >= 3) {
      insights.push(
        `${noSpendDays}/7 no-spend days this week – keep that streak alive!`
      );
    }
  }
  ...
  if (currImpulsive > prevImpulsive) {
    insights.push(
      `Oye, warning! You’ve gone overboard by ${formatINR(
        Math.round(currImpulsive - prevImpulsive)
      )} this month – Put a brake on those random purchases to protect your pocket! 😓`
    );
  }
  ...
  if (Array.isArray(currentDaily)) {
    ...
    if (mostExpensiveDay) {
      insights.push(
        `Most expensive day this month: ${mostExpensiveDay.date} – you went all out 😅`
      );
    }
  }
  ...
  if (weekendTravel > 0.5 * prevMonthTravel) {
    insights.push(
      `${formatINR(weekendTravel)} on Travel this weekend – could’ve been a staycation 🚕`
    );
  }
  ...
  if (Array.isArray(daily) && daily.length === 7) {
    insights.push(
      "You logged in daily for 7 days in a row – streak superstar! 🌟"
    );
  }
  ...
  if (Array.isArray(weeklyExpenses) && weeklyExpenses.length > 0) {
    const highestExpense = weeklyExpenses.reduce(
      (prev, curr) => (curr?.amount > (prev?.amount || 0) ? curr : prev),
      { amount: 0 }
    );
    insights.push(
      `Highest single spend this week: ${formatINR(highestExpense.amount)} on ${
  ...
  if (groceries?.amount) {
    ...
    const healthyTotal =
      (breakdown.fruits || 0) +
      (breakdown.vegetables || 0) +
      (breakdown.dairy || 0);
    if (healthyTotal / groceries.amount >= 0.15) {
      insights.push(
        `You spent ${formatINR(groceries.amount)} on groceries – home chef unlocked 🧑‍🍳`
      );
    }
  }
  ...
  if (subscription && monthlyTotal) {
    ...
    if (subPercent >= 8) {
      insights.push(
        "Subscription costs make up 8% of your monthly spending 📺"
      );
    }
  }
  ...
  if (weekendTotal > weekdayTotal * 2) {
    insights.push(
      "Your weekend spends are 2x your weekday spends – weekend warrior!"
    );
  }
  ...
  if (currMonthTotal > prevMonthTotal) {
    insights.push(
      `You have overspent by ${formatINR(
        Math.round(currMonthTotal - prevMonthTotal)
      )} compared to last month’s limit – stay sharp and win the month 💰`
    );
  }
  ...
  if (diningOut) {
    insights.push(
      `Monthly dining out cost: ${formatINR(diningOut)} – any thoughts to reduce that?`
    );
  }
  ...
  if (Bills > 3000) {
    insights.push(
      `Your utility bills are high this month: ${formatINR(Bills)} – it might be worth reviewing your usage or plans.`
    );
  }
  ...
  if (diningOut && grocerySpend) {
    ...
    if (dineOutRatio > 0.3) {
      insights.push(
        "More than half of your food spending is on dining out. Consider cooking at home more often to save money."
      );
    }
  }
...
async function headsUpMessages(userId)
{
  ...
  const currentWeektransactions = await getCustomDatesTransactions(newUserId, currentWeekStart, currentWeekEnd);
  const lastWeekTransactions = await getCustomDatesTransactions(newUserId, lastWeekStart, lastWeekEnd);
  ...
  const currentMonthTransactions = await getCustomDatesTransactions(newUserId, currentMonthStart, currentMonthEnd);
  const lastMonthTransactions = await getCustomDatesTransactions(newUserId, lastMonthStart, lastMonthEnd);
