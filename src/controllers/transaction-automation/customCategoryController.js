...
exports.addCustomCategory = async (req, res) => {
  ...
  try {
    let userCategory = await CustomCategory.findOne({ userId });
    ...
    if (!userCategory) {
      ...
      if (existingIndex !== -1) {
        ...
        // Add new category
        userCategory.categories.push({ name, imageUrl, narration: safeNarration });
      }
    }

    await userCategory.save();
    res.status(200).json({ message: "Category saved successfully", data: userCategory });
  } catch (err) {
