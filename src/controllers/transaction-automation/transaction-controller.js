...
const {AccountRepository, FipRepository} = require("../../repositories/index");
    ...
    if (!existingBankAccounts || existingBankAccounts.size == 0) {
      ...
      for (const bank of details) {
        ...
        if (hasAccount) {
          ...
          const fiObjectToUpdate = bank.fiObjects.find(fi =>
            fi.linkedAccRef === firstLinkRefNo || fi.linkedAccRef === accountRefNo
          );
          ...
          if (fiObjectToUpdate) {
            ...
            }, consenthandleid, userId, accountId);
