...
void changeTrasactiondata() async
{
          ...
          //  }
           ...
           allTransactions.sort((a, b) =>DateTime.parse(b["transactionTimestamp"]).compareTo(DateTime.parse(a["transactionTimestamp"])));
...
void  getAutoMationsTransactionsWeekly()async
{
      ...
      var response= await getDataApiCall("${url}/transactionauto/getalltransactionbyweek/${week}");
...
void  getAutoMationsTransactionsCustom(date,context)async
{
      ...
      var response= await getDataApiCall("${url}/transactionauto/getalltransactionsbycustom/${date}");
       trasactionsDataCreditWeekly.clear();
       trasactionsDataDebitWeekly.clear();
      if(getFlagOfResponse(response))
      {
...
void updateTheTagOfTarnsactions(source_category,destination_category,transactionId,context)async
{
    //  updateTransaction/:source_category/:destination_category/:transactionId
    String urlPath ="${url}/transactionauto/updateTransaction/${source_category}/${destination_category}/${transactionId}";
...
void hideTransactions(source_category,destination_category_name,transactionId,context)async
{
    String urlPath ="${url}/transactionauto/hideTransaction/${source_category}/${destination_category_name}/${transactionId}";
    var responce=await getDataApiCall(urlPath);
    if(getFlagOfResponse(responce)){
                 getAutoMationsTransactions();
    }
...
void unHideTransactions(source_category,destination_category_name,transactionId,context)async
{
    String urlPath ="${url}/transactionauto/unHideTransaction/${source_category}/${destination_category_name}/${transactionId}";
    var responce=await getDataApiCall(urlPath);
...
void getHideTransactions(context)async
{
    ...
    {       
        ...
        var  his=jsonDecode(response.body);
        trasactionsHideData.addAll(his['allTransactions']['categorized_transactions']);
