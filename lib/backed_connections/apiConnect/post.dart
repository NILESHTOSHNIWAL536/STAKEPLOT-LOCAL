    ...
    body:jsonEncode({
      ...
      if(response.statusCode==200 || response.statusCode==201){
            ...
            var snackBar = SnackBar(
                   ...
                   content: Text('Added Budget!',style:FontManager().getTextStyle(context,
                         color: Colors.white,
                         fontSize: 15,
           ...
           var snackBar = SnackBar(
                   ...
                   content: Text('invalid credentials!',style: FontManager().getTextStyle(context,
                         color: Colors.white,
                         fontSize: 15,
...
void  reportPost(context,String id,String spam)async{
    ...
    body: jsonEncode({
      ...
      if(response.statusCode==200 || response.statusCode==201){
            final body = json.decode(response.body);
             snackBarCalled(context,spam=="hide post"? "Post is hide From You":"Reported Successfully...!",Colors.green);
              // Navigator.pop(context);
              // Get.back();
      }else{
...
void createPostCopy(context,String title, String description, File imageFile) async {
...
void onUploadImage2(
    File selectedImage, BuildContext context, String title, String description) async {
    ...
    var res = await request.send();
     ...
     if (res.statusCode == 200) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Failed to upload image. Status code: ${res.statusCode}")),);
    }
  } catch (e) {
    // print("Error occurred: $e");
    ScaffoldMessenger.of(context).showSnackBar(
...
void addMessageImagePost(context,String name,String avatar,String id,File imageFile,String filenmae) async {
 ...
 Future<String> imageNameUrl = Future.value("");
  try {
    ...
    if (response.statusCode == 201 || response.statusCode == 200 ) {
      ...
      imageNameUrl=Future.value(data['data']['imageUrl']+"futureImagepathNileshBhaijan"+data['data']['filename']);
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Image uploaded successfully!")),
      );
    } else {
