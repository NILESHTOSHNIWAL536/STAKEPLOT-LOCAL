...
class AllCardsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Map<String, List<CardData>> groupedCards = {};
    for (var card in cards) {
      if (!groupedCards.containsKey(card.title)) {
        groupedCards[card.title] = [];
      }
      groupedCards[card.title]!.add(card);
    }
          ...
          child: Obx(() => cards.isEmpty
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, groupIndex) {
                    final groupTitles = groupedCards.keys.toList();
                    final group = groupedCards[groupTitles[groupIndex]]!;
                    return Padding(
                      padding: EdgeInsets.symmetric(vertical: verticalPadding),
                      child: SizedBox(
                        height: cardHeight,
                        child: Swiper(
                          ...
                          onIndexChanged: (index) {
                            if (index == group.length - 1 && group.length > 1) {
                              // Remove the swiped card from the list
                              cards.remove(group[0]);
                              onDataChanged();
                            }
                          },
