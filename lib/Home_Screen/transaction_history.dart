...
class _TransactionHistoryState extends State<TransactionHistory> {
  ...
  void showCustomFriendsModal2(BuildContext context, Map<String, dynamic> transaction) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      shape: const RoundedRectangleBorder(
                          ...
                          child: InkWell(
                            onTap: () async {
                              try {
                                ...
                                double sharePerFriend = amount / addedMembers.length;
                                print("Amount: $amount, Share Per Friend: $sharePerFriend");
                                splitUserAmount2(
                                  _stableContext ?? context,
                                  amount.toString(),
                                  addedMembers.toList(),
                                ...
                                ScaffoldMessenger.of(modalContext).showSnackBar(
                                  SnackBar(content: Text("Error splitting amount: $e")),
                                );
                              }
                            },
                            child: getButton(modalContext, "Continue"),
              ...
              children: [
                Expanded(
                  ...
                  child: Container(
                    ...
                    child: AvatarProfileImage(
                      url: Categories.link + (imageMapForHistory[category.toLowerCase()] ?? ''),
                      height: 16,
                      width: 20,
                    ),
                  ),
  ...
  //   });
  // }
  Future<http.Response> updateDataApiCall(String url, Map<String, dynamic> body) async {
  try {
  ...
  print("Hiding transaction at URL: $apiUrl");
  print("Current transaction state: $transaction");
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      print("Transaction $transactionId hidden successfully");
    } else {
      print("Failed to hide transaction. Status: ${response.statusCode}");
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Failed to hide transaction: ${response.statusCode} - ${response.body}")),
      );
    }
  } catch (e) {
    print("Error hiding transaction: $e");
    ScaffoldMessenger.of(context).showSnackBar(
  ...
  void getAllTransaction(BuildContext context) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("An error occurred while fetching transactions")),
      );
    }
  }
