...
class _TribeSearchState extends State<TribeSearch> {
  ...
  void getDis(data) async {
    final response =
        await getDataApiCall("${PostRoutes.userDiscussions}/${data['_id']}");
  ...
  void getStatus(data) async {
    ...
    if (widget.isMasked) {
      bool isMasked = userController.maskedConnections
          .any((friend) => friend['_id'] == data['_id']);
      if (!isMasked)
  ...
  void getTransaction() async {
    var response =
        await getDataApiCall("${UserRoutes.findFriend}/${widget.isMasked}");
      ...
      body: SafeArea(
        child: Container(
          ...
          padding: const EdgeInsets.symmetric(
              horizontal: 10, vertical: AppSizes.p30),
          child: ListView(
            children: [
              Hero(
                  tag: "TribeSearc",
        ...
        child: Center(
          child: TextFormField(
            ...
            onChanged: (value) {
              setState(() {
                if (value == "")
                  ...
                  frdsList = getLastTenUsers(
                      getSearchData(value, frdsListOrigin, widget.isMasked));
              });
            },
            decoration: InputDecoration(
              ...
              filled: true,
              contentPadding:
                  EdgeInsets.symmetric(vertical: 0, horizontal: AppSizes.p14),
              hintText: lableText,
  ...
  Widget profileContainer(data) {
    ...
    String name = widget.isMasked
        ? (data['maskedName'] ?? "")
        : (data['name'] ?? "Unknown User");
    String background = data['avatarBackGround'] ??
        defaultBackGround.value ??
        "#FFFFFF"; // Fallback to a default color
  ...
  Widget getScreen(data) {
    String avatar = data['avatarType'] != null
        ? data['avatarType']
        : data['avatar'] != null
            ? data['avatar']
            : ControllerManagement.userController.avatar;
    ...
    return Container(
      ...
      child: Column(
        children: [
          Container(
            margin: EdgeInsets.symmetric(vertical: AppSizes.p10),
          ...
          Row(
            ...
            children: [
              networkFriends(
                  "Network", count.toString(), Icons.person_2_outlined),
              AvatarProfile(
                name: name,
                width: 5,
                height: 10,
              ...
              networkFriends(
                  "Posts", getTrendingData.length.toString(), Icons.post_add),
            ],
          ),
          ...
          InkWell(
            onTap: () {
              if (buttonValue.value == "Remove") {
                ...
                if (widget.isMasked) {
                  if (ControllerManagement.userController.maskedName.value
                      .trim()
                      .isEmpty)
  ...
  Widget networkFriends(String network, String count, IconData icon) {
    return Column(
      children: [
        Container(
            padding:
                EdgeInsets.symmetric(vertical: 5, horizontal: AppSizes.p14),
            decoration: BoxDecoration(
              borderRadius: BorderRadius.circular(10),
