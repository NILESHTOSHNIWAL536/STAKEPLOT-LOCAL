...
Widget build(BuildContext context) {
        ...
        children: [
          ...
          SingleChildScrollView(
            ...
            child: Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.center,
              children: [
                // tribeHeader(context), // Uncomment if needed
                Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 15),
                  child: PostCard(data: widget.dataObj, flag: true),
                ),
                uploadData(widget.dataObj),
                SizedBox(
                  height: 100, // Space to avoid overlap with fixed input
                ),
              ],
            ),
          ),
          // Fixed comment/reply input box
          Positioned(
            left: 0,
            right: 0,
            bottom: 0,
            child: Container(
              ...
              child: replyid.isEmpty
                  ...
                  : InputComment(
                      "Reply to ${commentObj.commentText}",
                      TextInputType.text,
                      replyController,
                      replyid,
                    ),
 ...
 Widget commentedData() {
        ...
        children: [
          ...
          Obx(
            () => Container(
              ...
              decoration: BoxDecoration(
                borderRadius: BorderRadius.circular(12),
                border: Border.all(width: 0.3, color: AppColors.accentColor),
              ),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Column(
                    children: indexArray
                        .map((index) => dataComments(commentList[index], index))
                        .toList(),
                  ),
                  // replyid != ""
                  //     ? Padding(
  ...
  onTap: () {
    ...
    FocusScope.of(context).requestFocus(_replyNode); // Focus reply input
    _scrollToTextField(); // Scroll to bottom if needed
    autofocus.value = true;
  },
  child: Padding(
  ...
  void reply(String value, String commentId, TextEditingController textController) {
  if (value.isEmpty) {
    FocusScope.of(context).requestFocus(_replyNode);
    snackBarCalled(context, "You cannot add an empty reply.", Colors.red);
    return;
  }

  if (commentId.isEmpty) return;

  addReply(context, value, commentId); // Your existing reply API call
  Replies rep = Replies();
  ...
  setState(() {
    replyid = ""; // Reset reply mode
    textController.clear(); // Clear input
    autofocus.value = false; // Reset autofocus
  });
}
}
