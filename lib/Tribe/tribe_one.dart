...
class _TribeHomeState extends State<TribeUnique> {
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      setState(() {
        if (flag) {
          ...
          snackBarCalled(context,SnackbarData().commentLiked, Colors.black);
        } else {
          dataObj["upvotes"]--;
          if (dataObj["upvotes"] < 0) commentObj.upvotes = 0;
          postListIds.remove(objectId);
          snackBarCalled(context,SnackbarData().commentUnliked, Colors.black);
        }
      });
  ...
  void downvote(
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      final body = json.decode(response.body);
      snackBarCalled(context,SnackbarData().disliked, Colors.black);
      ...
      snackBarCalled(context,SnackbarData().errorDislikingComment, Colors.red);
    }
  }
