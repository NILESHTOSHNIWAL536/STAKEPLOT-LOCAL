  ...
  try {
    ...
    if (getFlagOfResponse(res)) {    
      ...
      controller.categoriesList.clear();
       ...
       controller.frequentPaymentsWeek.clear();
       controller.moreDrasticChangeWeek.clear();
       controller.mostSpentCategoryInMonth.clear();
       controller.mostSpentDayInMonth.clear();
      ...
      // frequentPayments.addAll(data["data"]['frequentPayments']);
      // moreDrasticChange.addAll(data["data"]['moreDrasticChange']);
       controller.frequentPayments.addAll(
        (data["data"]['frequentPayments'] as List<dynamic>)
            .map((e) => e as Map<String, dynamic>)
            .toList(),
      );

       controller.moreDrasticChange.addAll(
        (data["data"]['moreDrasticChange'] as List<dynamic>)
            .map((e) => e as Map<String, dynamic>)
            .toList(),
      );

       controller.totalDebitThisMonth.value = double.parse(
          doubleToFixed(data["data"]['totalDebitThisMonth'].toString()));

       controller.categoriesListWeek.addAll(
        (data["data"]['week']['categorized'] as List<dynamic>)
            .map((e) => e as Map<String, dynamic>)
            .toList(),
      );
       controller.frequentPaymentsWeek.addAll(
        (data["data"]['week']['frequentPayments'] as List<dynamic>)
            .map((e) => e as Map<String, dynamic>)
            .toList(),
      );
       controller.moreDrasticChangeWeek.addAll(
        (data["data"]['week']['moreDrasticChange'] as List<dynamic>)
            .map((e) => e as Map<String, dynamic>)
            .toList(),
      );
     controller.mostSpentCategoryInMonth.addAll(
  (data["data"]['mostSpentCategory'] as List<dynamic>)
      .map((e) => e as Map<String, dynamic>)
      .toList(),
);
if (data["data"]['mostSpentDay'] != null) {
   controller.mostSpentDayInMonth.add(
    data["data"]['mostSpentDay'] as Map<String, dynamic>,
  );
}
if (data["data"]['weeklyTrend'] != null) {
   controller.weeklyTrend.add(
    data["data"]['weeklyTrend'] as Map<String, dynamic>,
  );
}
      ...
      // Refresh reactive lists
       ...
       controller.frequentPayments.refresh();
       controller.moreDrasticChange.refresh();
       controller.categoriesListWeek.refresh();
       controller.frequentPaymentsWeek.refresh();
       controller.moreDrasticChangeWeek.refresh();
       controller.mostSpentCategoryInMonth.refresh();
       controller.mostSpentDayInMonth.refresh();
       controller.weeklyTrend.refresh();
